...
from mnist_dataloader import Dataset
...
def run():
    ...
    random_integers_labels = torch.randint(low=0, high=10, size=(16,), dtype=torch.int32).to(args.device)
    ...
    for epoch in range(args.epochs):
        ...
        for idx,(batch_train,label_train) in enumerate(train_loader):
            noise_batch = torch.randn((batch_train.shape[0],args.latent_dims,1,1)).to(args.device)
            batch_train = batch_train.to(args.device)
            label_train = label_train.to(args.device)
            
            fake_img = gen_model(noise_batch,label_train)
            fake_prediction = disc_model(fake_img,label_train).reshape(-1)
            real_prediction = disc_model(batch_train,label_train).reshape(-1)
            ...
            fake_pred = disc_model(fake_img,label_train).reshape(-1)
        ...
        gen_model.eval()
        with torch.no_grad():
            for idx_val , (batch_val,label_val) in enumerate(val_loader):
                noise_batch_val = torch.randn((batch_val.shape[0],args.latent_dims,1,1)).to(args.device)
                batch_val = batch_val.to(args.device)
                label_val = label_val.to(args.device)
                fake_img_val = gen_model(noise_batch_val,label_val)
                fake_prediction_val = disc_model(fake_img_val,label_val).reshape(-1)
                real_prediction_val = disc_model(batch_val,label_val).reshape(-1)
                ...
                fake_pred_val = disc_model(fake_img_val,label_val).reshape(-1)
                gen_loss_val = criterion(fake_pred_val, torch.ones_like(fake_pred_val))
                
                
            
            ## save_image_grid
            generated_fake_img = gen_model(noise,random_integers_labels)
            grid_16 = make_grid(generated_fake_img,nrow=4,normalize=True)
            save_image(grid_16 , f'./result/grid_epoch{epoch}.jpg')
